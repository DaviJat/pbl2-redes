...
import pickle
import os
from servidor1 import retornar_distancia_servidor_2, retornar_distancia_servidor_3
...
distancias_servidor2 = retornar_distancia_servidor_2()
distancias_servidor3 = retornar_distancia_servidor_3()

distancias_unificadas = {}

def adicionar_distancias(distancias):
    for cidades, dados in distancias.items():
        # Se a combinação de cidades já existir, você pode optar por somar ou escolher o melhor
        if cidades in distancias_unificadas:
            # Exemplo: somar as distâncias e quantidades
            distancias_unificadas[cidades] = (
                distancias_unificadas[cidades][0] + dados[0],
                distancias_unificadas[cidades][1] + dados[1]
            )
        else:
            distancias_unificadas[cidades] = dados

adicionar_distancias(distancias)
adicionar_distancias(distancias_servidor2)
adicionar_distancias(distancias_servidor3)
...
def criar_grafo():
    global grafo
    for (cidade1, cidade2), (distancia, passagens) in distancias_unificadas.items():
